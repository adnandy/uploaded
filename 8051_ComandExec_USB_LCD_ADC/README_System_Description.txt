...
-=============================================================-
The project describes how to interface a single low-end CMOS 8051 micro-chip with some peripherals: all working together as a system.

An 8-bit ADC, a USB port, a 16*2 LCD and a temperature sensor (for just an idea of interfacing any sensor) that will keep monitoring and storing the temperature at every 2 seconds time gap (by default) until it is not instructed to change its time gap. This device has some commands-execution feature that allows a user to sit in front of a computer and have control from back to perform commands. The device performs 2 types of commands.
...
As soon as the device is commanded with the computer, it will send an acknowledgement as “COMMAND ACCEPTED” for correct command and “WRONG COMMAND!” for every incorrect command input. If command is correct device will start executing that command immediately. When execution is finished device will go back to the process of monitoring and storing the temperature. In this way user can get a set of data chart for a certain time gap which he/she wants to observe. And will be able to identify the certain changes in temperature.
